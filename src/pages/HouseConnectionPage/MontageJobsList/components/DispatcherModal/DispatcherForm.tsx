...
import { Alert, Box, Button, DialogActions, TextField, Typography } from '@mui/material';
...
import { DispatcherFields } from 'types/montage-jobs';
import { JOB_STATUSES } from 'constants/montageJobs';
...
const DispatcherForm: React.FC<Props> = ({onSubmit, submitError, isLoading, onClose}) => {
  ...
  return (
    <Box component="form" onSubmit={handleSubmit(onFormSubmit)} sx={formStyles}>
        ...
        <SelectController
          ...
          options={
            JOB_STATUSES.map((jobStatus) => (
              { value: jobStatus, label: t(`jobStatus.options.${jobStatus}`) }
            ))
          }
      ...
      <DialogActions sx={{ position: 'absolute', bottom: '10px', right: '20px' }}>
        <Box sx={actionButtonWrapperStyles}>
          <Button variant="text" onClick={onClose}>{t('closeBtn')}</Button>
          <LoadingButton
            type="submit"
            variant="contained"
            fullWidth
